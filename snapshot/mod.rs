...
pub fn create_snapshot() {
    let ops_file = read_directory("js/").unwrap();
    ...
    for opf in ops_file {
        let op_file: String = opf.path().to_str().unwrap().to_string();
        let op_name = op_file.replace("js/", "").replace(".js", "");
    ...
    fs::write("snapshot.bin", snapshot).expect("failed to write snapshot the file: ./snapshot.bin");
